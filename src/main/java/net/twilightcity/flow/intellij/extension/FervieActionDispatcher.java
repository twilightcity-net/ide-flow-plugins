...
public class FervieActionDispatcher implements ActionDispatcher {
	...
	public FervieActionDispatcher(Logger log, FervieExtensionPointService fervieExtensionPointService, GotoFileActionHandler gotoFileActionHandler) {
		this.log = log;
	...
	public void dispatchAction(Action action) {
		if (action.getActionType().equals(ActionType.RUN)) {
			...
			this.fervieExtensionPointService.fireAction(runAction.getActionId(), createFlowStateContext(action.getFlowInsightActionContext()));
			...
			this.gotoFileActionHandler.fireAction(gotoAction.getModule(), gotoAction.getFilePath());
		}
	}
